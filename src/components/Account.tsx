...
const Account = () => {
  ...
  const dappFlowNetworkName = useMemo(() => {
    return algodConfig.network === ''
      ? 'sandbox'
      : algodConfig.network.toLowerCase();
  }, [algodConfig.network]);
  ...
  function ellipseAddress(activeAddress: string | undefined): import("react").ReactNode {
  ...
  return (
    <div className="h-full">
      ...
      <div className="text-xl">
        Network: {algodConfig.network === '' ? 'localnet' : algodConfig.network}
      </div>
    </div>
  );
};
